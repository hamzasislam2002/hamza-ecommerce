...
def generate_password_reset_email(user_email):
    ...
    password_reset_url = url_for('users.prcoess_password_reset_token',
                                 token=password_reset_serializer.dumps(user_email, salt='password-reset-salt'),
                                 _external=True)
    
    return Message(subject='Password Reset Requested', html=render_template('users/email_password_reset.html', password_reset_url=password_reset_url),
...
def confirm_email(token):
    try:
        ...
        current_app.logger.info(f'Imvalid or expired confirmation link recieved from IP address: {request.remote_addr}')
        return redirect(url_for('users.login'))
...
def generate_confirmation_email(user_email):
    confirm_serializer = URLSafeTimedSerializer(current_app.confog['SECRET_KEY'])
    confirm_url = url_for('users.confirm_email',
                          token=confirm_serializer.dumps(user_email, salt='email-confirmation-salt'),
                          _external=True)
    
    return Message(subject='Comfirm Your Email Address',
...
def password_reset_via_email():
    ...
    if form.validate_on_submit():
        ...
        if user.email_confirmed:
            ...
            flash('Your email address must be confirmed before attempting password reset.', 'error')
        return redirect(url_for('users.login'))
